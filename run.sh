...
for filename in ./specgen_input/06_DataModel/Custom/Common/*.xml; do
  ...
  xsltproc sifobject.xslt "$filename" | perl xslt_postprocess.pl | perl struct2cs.pl > sifxml/$(basename "$filename" .xml).cs
done
...
echo '</root>' >> data.xml
xsltproc sifobject.xslt data.xml | perl xslt_postprocess.pl | perl struct2cs.pl > sifxml/DataModel.cs
